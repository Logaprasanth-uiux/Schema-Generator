...
export const WorkflowNavigation: React.FC = () => {
  ...
  return (
    <nav
      aria-label="Pipeline"
      className="w-40 lg:w-48 bg-neutral-50/50 dark:bg-neutral-900/60 border-r border-neutral-200 dark:border-neutral-800 flex flex-col p-4 select-none shrink-0 h-full overflow-y-auto transition-colors duration-150"
    >
      {/* Minimal Header */}
          ...
          return (
            <div key={st.id} className="flex flex-col">
              ...
              <button
                ...
                className={`group flex items-center space-x-2.5 py-1.5 px-1 rounded-md text-left transition-all ${
              ...
              >
                ...
                <div
                  className={`h-6 w-6 rounded-full flex items-center justify-center font-mono text-xs shrink-0 transition-colors ${
                    isActive
                      ? 'bg-neutral-900 text-white dark:bg-white dark:text-neutral-900 font-bold shadow-xs ring-2 ring-neutral-900/10 dark:ring-white/10'
                      : isCompleted
                ...
                >
                  {isCompleted ? <Check className="h-3.5 w-3.5" /> : st.number}
                </div>

                {/* Stage Label — Always visible for Active Stage, subtle on hover for accessible stages */}
                {isActive ? (
                  <span className="text-xs font-semibold text-neutral-900 dark:text-white truncate">
                    {st.label}
                  </span>
                ) : (
                  <span className="text-xs text-neutral-400 dark:text-neutral-500 group-hover:text-neutral-700 dark:group-hover:text-neutral-300 truncate transition-colors">
                    {st.label}
                  </span>
                )}
              </button>
              ...
              {!isLast && (
                <div className="ml-3.5 my-0.5 w-px h-5 bg-neutral-200 dark:border-neutral-800 dark:bg-neutral-800" />
              )}
            </div>
          );
        })}
